...
### 6-B1. APIトークンの暗号化保存【セキュリティ】（Phase 8 A-1で実装済み）
...
### 6-B2. ポートフォリオダッシュボード（全案件横断ビュー）（Phase 8 B-1で実装済み）
...
### 6-B3. 週次サマリーの自動生成（Procrastinate定期タスク）（Phase 8 C-1で実装済み）
...
### 6-B4. グローバル検索（⌘K）の実装（Phase 8 B-2で実装済み）
...
### 6-B5. チケット履歴の取込と再オープン率（Phase 8 C-2で実装済み）
...
### 6-B8. 通知の外部連携（メール / Slack Webhook）（Phase 8 C-3で実装済み）
...
### 6-B9. LLMコスト・利用状況ダッシュボード（Phase 8 D-1で実装済み）
...
### 6-B10. レポートテンプレート管理と定期ドラフト（Phase 8 D-2で実装済み。定期ドラフト自動生成は未実装、テンプレート管理のみ）
